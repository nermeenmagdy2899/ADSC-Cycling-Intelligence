...
## 2. Stack & run
- **Stack:** React 19 · Vite 7 · TypeScript · Tailwind 3 (`darkMode:"class"`, `:root`=light / `.dark`=dark) · Zustand · framer-motion ^12 · GSAP ^3.13 · Lenis · ECharts 6.1 · mapbox-gl (no token → the bespoke **SVG map** in `GeoJsonRouteOverlay` is what renders — this is the intended star) · lucide-react. **Not installed:** three.js, anime.js, openai SDK.
...
- **Story** (`src/components/StoryExperience.tsx`): 6 tab-driven fly-in chapters — Vision + Value, Principles + Users, Strategy + Route Types, Network, Progress, Future + Road Ahead. Chapter navigation is independent from GIS selection; only explicit route/type/map/dashboard/assistant interactions change the selected route or visible layers. The `progress` chapter remains the main highlight and renders `ExecutiveDashboard.tsx` with Overview / Scope / Progress / Budget tabs; programme KPIs live there only.
...
| Executive dashboard restoration + deep QA | done | 2026-07-12T21:35:00+03:00 | GPT-5 Codex | `4dabdad` | Restored a four-view executive dashboard (Overview/Scope/Progress/Budget), removed duplicate KPI chapter/strip, linked package filters to map, expanded canvas, added UAE ambient and typography motion, upgraded assistant auto-scroll/typing/online/message motion, verified budget against PDF pages 2/15, upgraded ECharts 6.1, and completed 375/768/1280/1440/1900 EN/AR theme QA with zero known dependency vulnerabilities. |
| Story consolidation + selection independence | done | 2026-07-12T21:46:58+03:00 | GPT-5 Codex | `40687ea` | Consolidated ten narrative tabs into six without dropping source content, decoupled chapter navigation from route/layer state, preserved explicit GIS filtering, and added named-persona retrieval plus complete persona context for live/offline assistant responses. Verified Track 2B persistence across chapters and the exact Explorer query; TypeScript and production build pass. |
| Merged strategy card fit + presentation polish | done | 2026-07-12T22:32:46+03:00 | GPT-5 Codex | `2f579da` | Removed the duplicated strategy/type preview, consolidated it into four complete interactive route-family cards, increased the small-screen deck height, corrected merged-chapter responsive selectors, and added a subtle theme-safe hover light sweep. TypeScript and production build pass. |
| Dashboard hierarchy + future-card fit | done | 2026-07-12T22:53:07+03:00 | GPT-5 Codex | `e7c7a32` | Enlarged executive KPI typography, added four differentiated theme-safe accent glows, staggered KPI entrance motion, increased dashboard canvas height, removed the duplicated Future budget gauge, and verified zero dashboard overflow plus a 19px Future-card clearance at 598x794. TypeScript and production build pass. |
| Principles fit + immediate map pan | done | 2026-07-13T00:04:21+03:00 | GPT-5 Codex | `1bb48e4` | Rebalanced the merged Principles/User Groups chapter into compact two-column principles and persona grids, verified every item is visible, audited all six chapters with zero article overflow, and made the first map drag establish a modest working zoom so panning responds from full extent. Real pointer QA changed viewBox from `0 0 1000 620` to `0 19.7 840 520.8`; TypeScript/build pass. |
