...
- **Story** (`src/components/StoryExperience.tsx`): 10 tab-driven fly-in steps — `vision, value, principles, users, strategy, route-types, network, progress, future, ask`. The `progress` step is the main highlight and renders `ExecutiveDashboard.tsx` with Overview / Scope / Progress / Budget tabs; programme KPIs live there only.
